...
import org.opentorah.xml.{Attribute, ContentType, Element, From, Parser}
...
object Metadata {
  ...
  def load[M](
    ...
    val wrappedParser = for {
      ...
      result <- new Element[M](elementName, ContentType.Elements, parser) {
        override def toXml(value: M): Elem = ??? // TODO!
      }.all
    } yield result
