...
{ this: S#Point =>
  protected final override def newNumber(raw: RawNumber): S#Point = numberSystem.Point.newNumber(raw)
  ...
  final def -(that: S#Point): S#Interval = numberSystem.Interval.newNumber(add(negate = true, that))
}
