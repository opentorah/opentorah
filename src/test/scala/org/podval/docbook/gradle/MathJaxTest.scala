...
class MathJaxTest extends FlatSpec with Matchers {
  ...
  "NodeJS" should "be available" in {
    val nodeJS: NodeJS = NodeJS.createNodeJS()
    val version = nodeJS.getNodeVersion
    version.nonEmpty shouldBe true
  }

  "MathJaxNode" should "be available" in {
    val mathJax = getMathJax
    mathJax.open() // TODO doesn't seem to be needed?
  ...
  private def getMathJax: MathJax = new MathJax(getBuildDir)
