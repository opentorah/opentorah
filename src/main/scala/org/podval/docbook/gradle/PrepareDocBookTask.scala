...
import org.podval.docbook.gradle.util.Util.writeInto
...
class PrepareDocBookTask extends DefaultTask  {
  ...
  def prepareDocBook(): Unit = {
    ...
    // XSLT stylesheets
    // TODO - add to the output directories?
    ...
    // substitutions DTD
    writeInto(layout.xmlFile(layout.substitutionsDtdFileName), logger) {
    ...
    // XML catalog
    writeInto(layout.catalogFile, logger) {
  ...
  private def unpackDocBookXsl(stylesheets: Stylesheets, version: String): Unit = {
    ...
    if (!directory.exists) {
      ...
      val file: File = Gradle.getArtifact(getProject, dependencyNotation)
  ...
  private def writeMainStylesheet(
    ...
    writeInto(layout.stylesheetFile(mainStylesheetName), logger) {
      s"""${Xml.header}
         |<!-- DO NOT EDIT! Generated by the DocBook plugin. -->
...
//      .reduceLeft[Map[String, String]] { case (result, current) => result ++ current }
    ...
    writeInto(layout.stylesheetFile(paramsStylesheetName), logger) {
      s"""${Xml.header}
         |<!-- DO NOT EDIT! Generated by the DocBook plugin. -->
         |<xsl:stylesheet ${Namespace.Xsl} version="${docBook2.xsltVersion}">
         |$parametersStr
