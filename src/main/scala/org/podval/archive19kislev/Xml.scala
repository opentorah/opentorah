...
object Xml {
  ...
  def print(xml: Node, outStream: OutputStream, prefix: Seq[String]): Unit = print(xml, new OutputStreamWriter(outStream), prefix)
  def print(xml: Node, outFile: File, prefix: Seq[String] = Seq.empty): Unit = print(xml, new FileWriter(outFile), prefix)

  def print(xml: Node, writer: Writer, prefix: Seq[String]) {
