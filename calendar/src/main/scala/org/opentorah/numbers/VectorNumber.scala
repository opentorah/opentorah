...
/** Vector from the number system `S`.
  ...
  */
abstract class VectorNumber[S <: Numbers[S]](digits: Digits) extends Number[S, S#Vector](digits) { this: S#Vector =>
  ...
  final def *(n: Int): S#Vector = numbers.Vector.fromDigits(digits map (_ * n))
  ...
  final def *(that: BigRational, length: Int): S#Vector =
    numbers.Vector.fromRational(this.toRational*that, math.max(this.length, length))
