...
- [x] find a version of Saxon that can handle DocBook XSLT;
- [x] make the build composite to allow for the plugin development;
- [x] move docbook.gradle into an included build (for now);
- [x] migrate it to an included Gradle plugin;
- [x] add support for naming input document and rename 'index.xml' back to 'calendar.xml';
- [x] referencing DocBook XSL files via javax.xml.transform.setURIResolver();
- [x] define stylesheet parameters programmatically via javax.xml.transform.setParameter();
...
- [ ] make DocBook XSLT version configurable via docbook { xslVersion = ... }
     and avoid "can't change after... resolved" - like 'zinc' for scala-plugin;
- [ ] define flexible enough set of parameters;
- [ ] add support for multiple named documents;
- [ ] support SVG in DocBook;
- [ ] support MathML in DocBook;
- [ ] make MathML namespace global:
   https://stackoverflow.com/questions/44956861/docbook-5-add-mathml-namespace-to-html-output
...
- [ ] move the plugin into a separate repository.
