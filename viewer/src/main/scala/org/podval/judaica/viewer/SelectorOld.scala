...
final class SelectorOld(val what: String, val name: String) {
...
object SelectorOld {
    ...
    def maybeFromXml(xml: Node): Option[SelectorOld] =
        if (!isDiv(xml)) None else Some(fromXml(xml))


    def fromXml(xml: Node): SelectorOld = {
        if (!isDiv(xml)) throw new IllegalArgumentException("Not a div!")

        new SelectorOld(getType(xml), getName(xml))
    }
    ...
    def toName(selectors: Seq[SelectorOld]): String = selectors.map(_.toName).mkString("-")
}
