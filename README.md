...
There are differences in the output:
- main output file is not named `index.html` (unless `use.id.as.filename` is on and id of the main document is `index`?);
- no navigational links are generated in the chunks;
- bibliography and glossary are in the main chunk.

Plugin processing DocBook into HTML with the new stylesheets:
- `docBook` extension has `outputFormats2` property, which can be overridden from the command line with `docBook.outputFormats2`;
- HTML customization is in `src/main/xsl2/html.xsl` (and is auto-generated by the plugin);
- stylesheets themselves get unpacked under `build/docBookXsl2`;
- output is in `build/docBook2/html/`;
- overall output directory is available as `outputDirectory2` property on the `processDocBook` task.
