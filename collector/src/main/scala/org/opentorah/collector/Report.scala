...
abstract class Report[T](override val name: String, val title: String) extends Store with HtmlContent[Collector] {
  ...
  protected def lines(site: Collector): Caching.Parser[Seq[T]]

  protected def lineToXml(line: T, site: Collector): ScalaXml.Element
}
...
object Report {
  ...
  object NoRefs extends Report[WithSource[EntityReference]](
    ...
    override protected def lines(site: Collector): Caching.Parser[Seq[WithSource[EntityReference]]] = site.getReferences.map(_
      .filter(_.value.ref.isEmpty)
      .sortBy(reference => ScalaXml.toString(reference.value.name).toLowerCase)
    )

    override protected def lineToXml(reference: WithSource[EntityReference], site: Collector): ScalaXml.Element = {
  ...
  object Unclears extends Report[WithSource[Unclear.Value]](
    ...
    override protected def lines(site: Collector): Caching.Parser[Seq[WithSource[Unclear.Value]]] =
      site.getUnclears.map(_.sortBy(_.source))

    override protected def lineToXml(unclear: WithSource[Unclear.Value], site: Collector): ScalaXml.Element = {
  ...
  object MisnamedEntities extends Report[Entity](
    ...
    override protected def lines(site: Collector): Caching.Parser[Seq[Entity]] = site.entities.directoryEntries.map(_
      .filterNot(entity => entity.id == getExpectedId(entity))
      .sortBy(_.name)
    )

    override protected def lineToXml(entity: Entity, site: Collector): ScalaXml.Element =
