...
import org.opentorah.archive.collector.selectors.NameSelector
...
final class Names(
  ...
  val nameds: Seq[Named] = for (teiNamed <- storeNameds) yield {
    ...
    val namedPath = path :+ Binding.Named(NameSelector, new org.opentorah.metadata.Names(
