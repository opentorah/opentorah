...
final class Layout(val frameworksDir: File, val projectDir: File, val buildDir: File) {
  ...
  val catalogGroupBase: String = "../../.."  // to get to the project root
  ...
  private val buildDirRelative: String =
...
object Layout {
  abstract class ForDocument(prefixed: Boolean, documentName: String) {
    ...
    private def outputDirectory(variant: Variant, isSaxon: Boolean): File = new File(
      Files.prefixedDirectory(
        directory = root(isIntermediate(variant, isSaxon)),
