...
final class Sections(
  ...
  def parameters(variant: Variant): Seq[(String, Parameters)] =
    ...
    ) ++ (if variant.name.isEmpty then Seq.empty else Seq(
...
object Sections:
  ...
  def apply(parameters: Map[String, Parameters]): Sections =
    ...
    for (name: String, sectionParameters: Parameters) <- parameters do
      val (sectionName: String, variantName: Option[String]) = Strings.split(name, '-')
      if variantName.isEmpty then
        val docBook2: Option[DocBook2] = DocBook2.find(sectionName)
        if docBook2.isEmpty then commonSections += ((CommonSection.forName(sectionName), sectionParameters))
