...
import org.opentorah.math.{MathConfiguration, MathJax}
...
final class CalculatedParameters(
  ...
  def mathJaxConfigurationParameter: Map[String, String] = if !math.enableMathJax then Map.empty else Map(
    CalculatedParameters.mathConfigurationParameterName -> MathJax(math).htmlConfigurationString(math)
  )
  // TODO
  ...
  def mathJaxUserHeadContent: ScalaXml.Nodes = if !math.enableMathJax then Seq.empty else Seq(
      ...
      <xsl:template name="user.head.content">
        {MathJax(math).body(<xsl:value-of select={s"$$${CalculatedParameters.mathConfigurationParameterName}"}/>)}
      </xsl:template>
    )
