...
final class Chapter(override val number: Int, val length: Int) extends Store.Numbered with Store.NonTerminal with Stores.Pure {
  final class VersesBy extends By with Stores.Numbered[VerseStore] {
    override def selector: Selector = Selector.byName("verse")
    override def length: Int = Chapter.this.length
    override protected def createNumberedStore(number: Int): VerseStore = new VerseStore(number)
  }
