...
final class ICal(os: OutputStream):
  private val out: PrintStream = PrintStream(os)
...
object ICal:
  ...
  def print(properties: Properties, out: PrintStream): Unit = properties.foreach((name, value) =>
  ...
  def beginEvent(transparent: Boolean): Properties = Seq(
    ...
    "CLASS" -> "PUBLIC",
    "STATUS" -> "CONFIRMED",
    "TRANSP" -> (if transparent then "TRANSPARENT" else "OPAQUE")
  )
  ...
  def event(
    ...
    event(transparent = true, properties =
      summary(title) ++
      fullDayDuration(day) ++
      googleContent(title, icon, url, width, height)
    )

  def fullDayDuration(day: Day): Properties =
    def toDate(day: Day): String =
      def pad2digits(what: Int): String = (if what < 10 then "0" else "") + what
