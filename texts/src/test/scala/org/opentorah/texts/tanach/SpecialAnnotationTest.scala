...
final class SpecialAnnotationTest extends AnyFlatSpec, Matchers:
  ...
  "Pesach day 1" should "give back hamichlol's reading as a variant" in:
    val variants = ReadingSources.variantsForSpecialReading("Pesach1", Custom.Ashkenaz)
    ...
    ReadingSources.forSpecialReading("Pesach1", Custom.Ashkenaz)
  ...
  "the city of Algiers" should "give back the note that Asulin differs" in:
    val comment = ReadingSources.commentForSpecialReading(
      "Fast", Custom.Algiers, when = Some("afternoon"), role = Some("default")).get
  ...
  "Agadir" should "give back what it reads at Tisha BeAv" in:
    ReadingSources.commentForSpecialReading(
      "TishaBeAv", Custom.Agadir, when = Some("afternoon")).get should include ("without the")

  "a reading with nothing recorded" should "simply say so" in:
    ReadingSources.forSpecialReading("RoshHashanah1", Custom.Ashkenaz) shouldBe empty
    ReadingSources.commentForSpecialReading("RoshHashanah1", Custom.Ashkenaz) shouldBe None
  ...
  "an annotation" should "not leak onto the customs sharing its entry" in:
    ...
    // Asulin, which is the whole reason it is reads="inherit" and not a comment
    def comment(custom: Custom): String =
      ReadingSources.commentForSpecialReading(
        "Fast", custom, when = Some("afternoon"), role = Some("default")).get
